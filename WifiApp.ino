...
String stringInt(int number)
{
	if(number < 9)
	{
		return padNum("00", number);
	}

	if(number > 9 && number < 100)
	{
		return padNum("0", number);
	}

	return padNum("", number);
}

String padNum(String pad, int number)
{
	return pad + number;
}
...
/*
* Utlity functions ends
*/

//RX/TX to ESP8266 Module
...
struct Cycle
{
	...
	unsigned int total;
	const unsigned int iterator = 4;
};

//LED manager structure handle the brightness and its persistance
struct Led
{
...
//Relay device manager  structure
struct Relay
{
...
//Change the brightness of a LED
...
//LED handle brightness main function
...
//Set Level
void setLevel(void *device, int level, int index)
{
	Led* led = (Led*) device;
	led->cycle.level[index] = level;
}

//Get Level
int getLevel(void *device)
{
	Led* led =  (Led*) device;
	return *(led->cycle.level);
}

void parseLevelRequest(void *device, String levelData)
{
	Led* led = (Led*) device;
	int startIndex = 0;
	int endIndex = 3;
	for(int i = 0; i < led->cycle.total; i++)
	{
		setLevel(led, levelData.substring(startIndex, endIndex).toInt(), i);
		startIndex += led->cycle.iterator;
		endIndex += led->cycle.iterator;
	}
}

String deParseLevel(void *device)
{
	Led* led = (Led*) device;
	int last = (led->cycle.total - 1);
	String data = "";
	...
	for(int i = 0; i < led->cycle.total; i++)
	{
		if(i == last)
		{
			data = data + stringInt(led->cycle.level[i]);
		}
		else
		{
			data = data + stringInt(led->cycle.level[i]) + "|";
